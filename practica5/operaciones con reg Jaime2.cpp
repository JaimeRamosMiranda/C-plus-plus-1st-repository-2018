...
int buscarxcodigo(empleado fijo[],int n,int codigo)
 {int p,i;
  ...
  for(i=0;i<n;i++)//bucle para comparar el valor ingresado en fijo[ ].cod con el codigo
  					...
  					//para el caso 5 en main
  					//5 buscar codigo tiene 3 argumentos array (tipo empleado) per, tamaño del array n toma el valor final de cargar: n=n+1
          			//codigo que se ingresa aqui(buscarxcodigo(per,n,codigo)) será utilizado en la comparacion
      if(fijo[i].cod==codigo)
         p = i;// si se encuentra algun valor del array fijo[i].cod que coincida con el valor ingresado codigo=for[n].cod, se regresa un p diferente a -1
         		//en el caso 5 de Principal, el valor ingresado codigo es diferente
         		//p se retornara con el valor i del indice que coincide, en main regresara para el valor de pos
      return p;
} 
...
int buscarxnombre(empleado fijo[],int n,char nomb[30])
 {int p,i;
  p=-1;
  for(i=0;i<n;i++)
  		//6 buscar por nombre tiene 3 argumentos array (tipo empleado) per, tamaño del array n toma el valor final de cargar: n=n+1
        //nb que se ingresa aqui será utilizado en la comparacion
        //pos regresa el valor de i de la funcion, indice del registro cuyo codigo coincide con el aqui ingresado (i=0;i<n;i++)
   if(strcmp(fijo[i].nom,nomb)==0)//strcmp compara cada valor de fijo[i].nom, con nomb :cadena nb; si la comparacion es 0 (verdad), se procede
         p = i;//p se retornara con el valor i del indice que coincide, en main regresara para el valor de pos
      return p;
} 
...
//es usado en caso 5 y 6
void mostraruno(empleado f)//5 y 6 mostrar uno, utiliza un argumento array tipo empleado per, con indice [pos]
							//en este caso f=per[pos], por eso no se necesita indicar el indice de f, porque es unico
...
//8 ordena alfabetico
void ordenaalfabetico(empleado fijo[],int n)//8 orden alfabetico tiene 2 argumentos array (tipo empleado) per
 											//tamaño tamaño del array n toma el valor final de cargar: n=n+1
   {int i,j;//contadores
    empleado t;//variable tipo empleado t, es usado como almacen temporal
    for(i=0;i<n;i++)//metodo de la burbuja para cadenas
       for(j=i+1;j<n;j++)
        if(strcmp(fijo[i].nom,fijo[j].nom)>0)//se compara cadenas con strcmp el primero con el segundo para el campo .nom
        									//si la comparacion es mayor a 0 (diferentes), se procede
          {t=fijo[i];//t guarda el valor del primero
           fijo[i]=fijo[j];//el primero adquiere el valor del segundo
           fijo[j]=t;//el segundo adquiere el valor del primero
           			//se han intercambiado los valores del primero con el segundo
           			//(b.a)>0
           			//t=b
           			//fijo[i]=a
           			//fijo[j]=t=b
...
char depa[20],nb[max];//depa para ingresar el departamento a buscar case 4; nb para ingresar el nombre a buscar case 6
      ...
      fflush(stdin);
       gets(depa);//se definio la variable depa [20] en main
        ...
        case 5:system("cls"); 
          ...
          pos=buscarxcodigo(per,n,codigo);//5 buscar por codigo (se utilizo en 1 cargar)
          //5 buscar codigo tiene 3 argumentos array (tipo empleado) per, tamaño del array n toma el valor final de cargar: n=n+1
          //codigo que se ingresa aqui será utilizado en la comparacion
          //pos regresa el valor de i de la funcion, indice del registro cuyo codigo coincide con el aqui ingresado (i=0;i<n;i++)
        if(pos>=0)
           mostraruno(per[pos]);//5 y 6 mostrar uno, utiliza un argumento array tipo empleado per, con indice [pos]
        else    
           cout<<"el codigo del producto no existe"<<endl; 
           break; 
 case 6: system("cls");
 ...
 case 7 : modificacion(per,n);
         ...
         break;           
 case 8:ordenaalfabetico(per,n);break; //8 orden alfabetico tiene 2 argumentos array (tipo empleado) per
 										//tamaño tamaño del array n toma el valor final de cargar: n=n+1
 										//para ver los resultados se ejecuta 2 mostrar(per,n)
